...
with st.sidebar:
    ...
    max_new_tokens = st.slider(
        "Max output tokens", min_value=100, max_value=4000, value=600, step=100
    )
    decoding_method = st.radio(
        "Decoding", (DecodingMethods.GREEDY.value, DecodingMethods.SAMPLE.value)
    )
    parameters = {
    ...
    uploaded_file = st.file_uploader(
        ...
        type=["pdf", "docx", "txt", "pptx", "csv", "json", "xml", "yaml", "html"],
    )
